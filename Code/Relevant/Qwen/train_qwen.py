...
# Dataset sizes (used when building balanced splits)
...
def make_splits(jsonl_path: str, dataset_root: str):
    ...
    if EVAL_ONLY and JSONL_DIRECT:
        ...
        # If records contain a "split" field, filter by SPLIT_ENV; else, use all as the "test" set.
        ...
        return [], [], use
    # --- END DIRECT EVAL SHORT-CIRCUIT ---

    # Original balanced-split builder
    basename_index = None
    sens, nons = [], []
    missing = 0
    for rec in read_jsonl(jsonl_path):
...
def main():
    ...
    for i, rec in enumerate(test_ds):
        ...
        # Optional debug per-sample output (countable by grep to show progress)
        ...
        # Lightweight progress line (flush for live streaming)
